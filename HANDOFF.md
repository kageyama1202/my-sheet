...
- shared-modal.css — 共通モーダルスタイル。モーダル内は白背景+濃い文字色(color:#333)。
- shared-modal.js — 共通モーダルロジック。4段階マッチ（手動/KW/名前/TEL）。通信履歴+メモ+カンバン+保存コールバック。
...
app_tasks/
  [手配ID（csvData[0]ベースのsafeKey）]/  ← 2026-04-01にキー方式変更
...
app_communications/
  [自動キー]/
    dt: "2026-03-28 12:39:43" — 日時
    src: "imessage" or "gmail" — ソース
    dir: "sent" or "received" — 方向
    contact: "+818057244095" or "name <email>" — 送信先/元
    subject: "" — 件名（Gmail）
    body: "テキスト" — 本文（500文字制限）
    assignedCase: "手配IDキー" — 手動紐付け済み案件
...
app_parked_mails/
  [自動キー]/ — 仮置きメール（comm.htmlから送る機能は現在未実装）
```
...
1. **電話番号マッチ**: メッセージの電話番号 → phoneToCasesList[phone] で案件一覧取得
2. **名前マッチ（フォールバック）**: 連絡先名 → extractSei()で姓を抽出 → nameToCases[sei]で案件検索
3. **工務担当者クロスリンク**: csvData[8]の工務担当者名 → nameToPhones[name]で電話番号逆引き → phoneToCasesListに追加

### 非表示管理
- localStorageに保存（comm_hidden）
- 「👁 非表示を表示」トグルで薄く表示→戻せる
...
- 旧キー形式（cols.join全結合ベース）からの自動マイグレーション
...
2. メーラーページ（定型文テンプレート3種+送信先リスト）

### 中優先度
3. マップのジオコーディング完走確認（124件中75件程度取得済み）
4. 施工条件メモの構造化（app_contractor_notesの表示UI）
5. 連絡先のFirebase取り込み改善（Mac連絡先の名前クリーニング精度）
6. shared.js共通化の続き（モーダル以外のユーティリティ関数）

### 低優先度
7. AI下見スケジュール提案ページ
8. メール定型文テンプレート実装（3種：リフォーム問合/施工日確定タカラ/施工日確定先方SMS）
9. 担当者マスターシート
10. キーワードタブのクロスデバイス共有（現在localStorage）
11. Google Drive API連携（ファイル自動アップロード）
